...
import { AdminChatRoomView } from '@/components/admin-chat/AdminChatRoomView';
import { useAdminInternalChat } from '@/hooks/useAdminInternalChat';
...
export default function AdminInternalChat() {
  const {
    ...
    createDirectChat,
    createGroupChat,
  } = useAdminInternalChat();

  const isMobile = useIsMobile();
  const [mobileView, setMobileView] = useState<MobileView>('sidebar');

  // Initialize presence tracking for admins
  useAdminPresence();
  ...
  const handleLeaveGroup = async (_roomId: string) => {
