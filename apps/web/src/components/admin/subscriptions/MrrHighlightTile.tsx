...
interface MrrHighlightTileProps {
  /** Hero number — monthly recurring revenue from active subscriptions (BDT). */
  mrr: number;
  loading?: boolean;
}

/**
 * The single full-orange surface on the Subscriptions page (DESIGN_SYSTEM §7).
 * Focal KPI for MRR — orange stays rare so it stays loud.
 */
export function MrrHighlightTile({ mrr, loading = false }: MrrHighlightTileProps) {
  const display = useCountUp(mrr);
    ...
    <m.div variants={staggerItem} whileHover={{ y: -2 }} transition={{ duration: 0.15 }} className="h-full">
      <div className="relative flex h-full min-h-[120px] flex-col overflow-hidden rounded-card bg-primary p-5 text-primary-foreground shadow-elevation-2">
        ...
        <div className="relative z-10 flex h-full flex-col">
          ...
          <p className="mt-1 text-xs text-primary-foreground/75">from active subscriptions</p>
        </div>
      </div>
    </m.div>
  );
