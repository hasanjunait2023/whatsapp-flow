 ...
 export function useGroupMessages(groupId: string | null) {
   ...
   const { data: messages = [], isLoading, error, refetch } = useQuery({
     queryKey: ['group-messages', group?.wa_group_id],
     queryFn: async () => {
   ...
   useEffect(() => {
     if (!group?.wa_group_id || !tenant?.id) return;
 
     const channel = supabase
       .channel(`group-messages-${group.wa_group_id}`)
       .on(
         'postgres_changes',
         {
           event: 'INSERT',
         ...
         () => {
           queryClient.invalidateQueries({ queryKey: ['group-messages', group.wa_group_id] });
         }
       )
       .subscribe();
   ...
   }, [group?.wa_group_id, tenant?.id, queryClient]);
