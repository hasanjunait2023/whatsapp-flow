 ...
 * `vite.content.config.ts`) so AI crawlers (GPTBot, PerplexityBot, ClaudeBot,
 * Google-Extended) and Googlebot can read the content + JSON-LD in the INITIAL
 * HTML response. This build is fully isolated from the SPA app shell
 * (`src/App.tsx`) — no auth, no Supabase, no providers — so nothing
 * browser-only runs at build time.
 */
