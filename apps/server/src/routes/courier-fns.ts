 ...
 * checks are implemented; Pathao (OAuth) is recognised but booking is deferred.
 */
...
async function bookOne(
  ...
  if (courier === "pathao") {
    return { success: false, order_id: orderId, error: "Pathao booking is not available yet — use Steadfast" };
  }
...
export const COURIER_HANDLERS: Record<string, FnHandler> = {
  ...
  "courier-save-integration": async (body, ctx) => {
    ...
    const existing = sqlite
      .prepare("SELECT id, api_key, api_secret FROM courier_integrations WHERE tenant_id = ? AND provider = ? LIMIT 1")
      .get(ctx.tenantId, provider) as IntegrationRow | undefined;
      ...
      sqlite
        .prepare(
          `UPDATE courier_integrations
             SET api_key = ?, api_secret = ?, store_id = ?, is_active = ?, default_pickup_address = ?, updated_at = ?
           WHERE id = ?`,
        )
        .run(encKey, encSecret, storeId, isActive, pickup, now, existing.id);
    } else {
      sqlite
        .prepare(
          `INSERT INTO courier_integrations
             (id, tenant_id, provider, api_key, api_secret, store_id, is_active, default_pickup_address, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        )
        .run(crypto.randomUUID(), ctx.tenantId, provider, encKey, encSecret, storeId, isActive, pickup, now, now);
    }
