...
export function enqueueJob(opts: EnqueueOptions): string {
  const runAt = opts.runAt ?? new Date().toISOString();
  const payload = opts.payload === undefined ? null : JSON.stringify(opts.payload);

  if (opts.dedupeKey) {
    const existing = sqlite
      .prepare(`SELECT id FROM job_queue WHERE dedupe_key = ? AND status = 'queued' LIMIT 1`)
    ...
    if (existing) {
      sqlite
        ...
        .run(payload, runAt, new Date().toISOString(), existing.id);
      return existing.id;
    }
  }
  ...
  sqlite
    ...
    .run(id, opts.kind, opts.tenantId ?? null, payload, runAt, opts.dedupeKey ?? null);
  return id;
}
...
export async function processDueJobs(): Promise<number> {
  ...
  for (;;) {
    ...
    if (!handler) {
      sqlite
        .prepare(`UPDATE job_queue SET status = 'failed', last_error = ?, updated_at = ? WHERE id = ?`)
        .run(`No handler for kind "${job.kind}"`, new Date().toISOString(), job.id);
      continue;
    }
    ...
    try {
      ...
      sqlite
        .prepare(`UPDATE job_queue SET status = 'done', updated_at = ? WHERE id = ?`)
        .run(new Date().toISOString(), job.id);
    } catch (err) {
      ...
      if (attempts < MAX_ATTEMPTS) {
        ...
        sqlite
          ...
          .run(attempts, message, retryAt, new Date().toISOString(), job.id);
      } else {
        sqlite
          .prepare(
            `UPDATE job_queue SET status = 'failed', attempts = ?, last_error = ?, updated_at = ? WHERE id = ?`,
          )
          .run(attempts, message, new Date().toISOString(), job.id);
      }
    }
  }
  return processed;
