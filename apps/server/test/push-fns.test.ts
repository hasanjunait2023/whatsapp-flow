...
async function endpointCount(endpoint: string): Promise<number> {
  const row = (await dbGet(
    `SELECT COUNT(*)::int AS n FROM push_subscriptions WHERE endpoint = ?`,
    endpoint,
  )) as { n: number };
  return row.n;
}
...
beforeAll(async () => {
  ...
  await saveSubscription(TENANT_A, "user-a", {
    endpoint: ENDPOINT,
...
describe("push-unsubscribe authorization", () => {
  it("is a no-op for a different tenant (no cross-tenant delete)", async () => {
    ...
    expect(await endpointCount(ENDPOINT)).toBe(1); // still there
  });
  ...
  it("is a no-op for a different user in the same tenant", async () => {
    ...
    expect(await endpointCount(ENDPOINT)).toBe(1);
  });
  ...
  it("removes the subscription for its owner", async () => {
    ...
    expect(res.error).toBeNull();
    expect(await endpointCount(ENDPOINT)).toBe(0);
  });
});
