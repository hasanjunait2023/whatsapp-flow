...
describe("WAHA webhook ingest", () => {
  ...
  it("ingests an inbound message: contact + message + thread-state rows", async () => {
    ...
    const contact = sqlite
      .prepare("SELECT id, unread_count, phone_number FROM contacts WHERE instance_id = ?")
      .get(INSTANCE) as { id: string; unread_count: number; phone_number: string };
    expect(contact.phone_number).toBe("15551234567");
    expect(contact.unread_count).toBe(1);

    const message = sqlite
      .prepare("SELECT direction, status, content FROM messages WHERE wa_message_id = ?")
      .get("wamid-1") as { direction: string; status: string; content: string };
    expect(message.direction).toBe("inbound");
    expect(message.status).toBe("delivered");
    expect(message.content).toBe("hello");

    const thread = sqlite
      .prepare("SELECT unread_count, total_messages, last_message_direction FROM contact_thread_state WHERE contact_id = ?")
      .get(contact.id) as { unread_count: number; total_messages: number; last_message_direction: string };
    expect(thread.unread_count).toBe(1);
    expect(thread.total_messages).toBe(1);
    expect(thread.last_message_direction).toBe("inbound");

    const stats = sqlite
      .prepare("SELECT inbound_count, wa_inbound, new_conversations FROM tenant_daily_stats WHERE tenant_id = ?")
      .get(TENANT) as { inbound_count: number; wa_inbound: number; new_conversations: number };
    expect(stats.inbound_count).toBe(1);
    expect(stats.wa_inbound).toBe(1);
    expect(stats.new_conversations).toBe(1);

    const usage = sqlite
      .prepare("SELECT messages_received FROM usage_counters WHERE tenant_id = ?")
      .get(TENANT) as { messages_received: number };
  ...
  it("transitions message status via message.ack (sent → delivered → read)", async () => {
    ...
    await ack(2);
    expect(
      (sqlite.prepare("SELECT status FROM messages WHERE wa_message_id = ?").get("wamid-ack") as { status: string }).status,
    ).toBe("sent");

    await ack(3);
    const delivered = sqlite
      .prepare("SELECT status, delivered_at FROM messages WHERE wa_message_id = ?")
      .get("wamid-ack") as { status: string; delivered_at: string | null };
    expect(delivered.status).toBe("delivered");
    expect(delivered.delivered_at).not.toBeNull();

    await ack(4);
    const read = sqlite
      .prepare("SELECT status, read_at FROM messages WHERE wa_message_id = ?")
      .get("wamid-ack") as { status: string; read_at: string | null };
  ...
  it("updates whatsapp_instances status on session.status WORKING", async () => {
    ...
    expect(
      (sqlite.prepare("SELECT status FROM whatsapp_instances WHERE id = ?").get(INSTANCE) as { status: string }).status,
    ).toBe("disconnected");
    ...
    const inst = sqlite
      .prepare("SELECT status, phone_number, qr_code FROM whatsapp_instances WHERE id = ?")
      .get(INSTANCE) as { status: string; phone_number: string | null; qr_code: string | null };
  ...
  it("records every event in webhook_events_log", async () => {
    const count = sqlite
      .prepare("SELECT COUNT(*) AS n FROM webhook_events_log WHERE instance_id = ?")
      .get(INSTANCE) as { n: number };
