...
beforeAll(async () => {
  ...
  await dbRun(
    "INSERT INTO whatsapp_groups (id, tenant_id, instance_id, name, wa_group_id) VALUES ('g-a', ?, 'inst-a', 'Group A', '123@g.us')",
    TA,
  );
});

beforeEach(async () => {
  await dbRun("DELETE FROM group_add_queue");
  await dbRun("DELETE FROM tenant_daily_group_limits");
  await dbRun("DELETE FROM whatsapp_group_participants");
  vi.restoreAllMocks();
});
...
describe("bulk group-add enqueue", () => {
  ...
  it("records a queue row for an owned group", async () => {
    ...
    const row = (await dbGet("SELECT status, batch_size FROM group_add_queue WHERE id = ?", data.queue_id)) as {
      status: string;
      batch_size: number;
    };
...
describe("bulk group-add worker (paced + daily-capped)", () => {
  it("processes one batch, records participants, and bumps the daily quota", async () => {
    ...
    const q = (await dbGet("SELECT processed_count, status FROM group_add_queue")) as {
      processed_count: number;
      status: string;
    };
    expect(q.processed_count).toBe(2);
    expect(q.status).toBe("processing");
    const added = (await dbGet("SELECT COUNT(*)::int AS n FROM whatsapp_group_participants")) as { n: number };
    expect(added.n).toBe(2);
    const quota = (await dbGet("SELECT members_added FROM tenant_daily_group_limits WHERE tenant_id = ?", TA)) as {
  ...
  it("stops at the per-tenant daily limit", async () => {
    ...
    await dbRun(
      "INSERT INTO tenant_daily_group_limits (id, tenant_id, date, max_daily_limit, members_added) VALUES ('lim', ?, ?, 5, 4)",
      TA,
      today,
    );
    await groupQueueBatch(
