...
describe("llm registry", () => {
  it("throws when nothing is configured", () => {
    expect(() => resolveLlm(TENANT)).toThrow("No LLM provider configured");
  });

  it("falls back to the platform env default", () => {
    process.env.LLM_DEFAULT_PROVIDER = "gemini";
    process.env.LLM_DEFAULT_MODEL = "gemini-2.0-flash";
    process.env.GEMINI_API_KEY = "env-key";
    const resolved = resolveLlm(TENANT);
  ...
  it("uses the plan default over the platform default", () => {
    process.env.LLM_DEFAULT_PROVIDER = "gemini";
    process.env.GEMINI_API_KEY = "env-key";
  ...
  it("uses tenant BYOK settings above everything and decrypts the key", () => {
    process.env.LLM_DEFAULT_PROVIDER = "gemini";
    process.env.GEMINI_API_KEY = "env-key";
  ...
  it("uses tenant provider choice on the platform key when not BYOK", () => {
    process.env.OPENAI_API_KEY = "platform-openai";
    db.insert(llmSettings).values({ tenant_id: TENANT, provider: "openai" }).run();
    const resolved = resolveLlm(TENANT);
    expect(resolved.provider.name).toBe("openai");
...
describe("llm usage + budget", () => {
  it("records usage events with cost", () => {
    recordUsage(TENANT, "hermes", "openai", "gpt-4o-mini", {
    ...
    expect(tokensUsedThisMonth(TENANT)).toBe(2_000_000);
  });
  ...
  it("passes the budget check when under budget or no budget is set", () => {
    expect(() => checkBudget(TENANT)).not.toThrow();
    db.insert(llmSettings).values({ tenant_id: TENANT, monthly_token_budget: 10_000 }).run();
    recordUsage(TENANT, "soul", "gemini", "gemini-2.0-flash", {
      promptTokens: 4000,
  ...
  it("throws BudgetExceededError once the budget is consumed", () => {
    db.insert(llmSettings).values({ tenant_id: TENANT, monthly_token_budget: 5000 }).run();
    recordUsage(TENANT, "hermes", "gemini", "gemini-2.0-flash", {
      promptTokens: 4000,
      completionTokens: 1000,
    });
    expect(() => checkBudget(TENANT)).toThrow(BudgetExceededError);
  });

  it("does not count another tenant's usage", () => {
    db.insert(llmSettings).values({ tenant_id: TENANT, monthly_token_budget: 5000 }).run();
    recordUsage("other-tenant", "hermes", "gemini", "gemini-2.0-flash", {
      promptTokens: 9000,
