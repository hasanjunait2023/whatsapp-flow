...
interface FBContactLabelsDialogProps {
  open: boolean;
...
export default function FBContactLabelsDialog({
  open,
  onOpenChange,
  contactId,
  contactName,
}: FBContactLabelsDialogProps) {
  const {
    labels,
    labelsLoading,
    loading,
  ...
  const handleToggleLabel = async (labelId: string) => {
    ...
    try {
      if (hasLabel) {
        await removeLabelFromFBContact(contactId, labelId);
        setContactLabels((prev) => prev.filter((l) => l.id !== labelId));
      } else {
        await addLabelToFBContact(contactId, labelId);
  ...
  const handleCreateLabel = async () => {
    ...
    try {
      ...
      await addLabelToFBContact(contactId, newLabel.id);
