...
const wrap = (p) => `@amijunait ${BRAND}${p.replace(/@amijunait/g,'').trim()}`;

const DEFAULT_CONTENT = {
  slug: 'cod-tip-1',
  caption: 'COD return কমানোর ১টা সহজ নিয়ম 👇 #ecommerce #bangladesh #ecomex',
  clips: [
    'ক্যামেরার দিকে তাকিয়ে বলছে: "বাংলাদেশে অনলাইন সেলারদের সবচেয়ে বড় সমস্যা — COD অর্ডার ফেরত আসে। কারণ কাস্টমার confirm করে না।" ছোট অনলাইন শপের পরিবেশ।',
    'হাসিমুখে বলছে: "একটা ছোট নিয়ম — অর্ডারের পরেই একটা confirmation মেসেজ পাঠান। এতে fake অর্ডার অনেক কমে যায়।" আত্মবিশ্বাসী।',
  ],
};
...
async function genClip(prompt, idx){
  ...
  await page.keyboard.type(' ' + wrap(prompt).replace('@amijunait','').trim(), { delay: 10 });
...
const files=[];
for(let i=0;i<content.clips.length;i++){ const f=await genClip(content.clips[i], i+1); if(f) files.push(f); await page.waitForTimeout(1500); }
await browser.close();
log('generated clips:', files.length);
if(files.length===0){ log('no clips; abort'); process.exit(1); }
...
// brand: Ecomex logo overlay (bottom-center)
const branded=`out/${content.slug}-branded.mp4`;
execFileSync(FF,['-y','-i',stitched,'-i',LOGO,'-filter_complex','[1]scale=300:-1[lg];[0][lg]overlay=(W-w)/2:H-h-70','-c:a','copy',branded],{stdio:'ignore'});
log('BRANDED ->', branded);
